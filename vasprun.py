...
def read_kpoints(kpts_file):
    ...
    for i in range(4,len(lines)):
        ...
        if tmp == "":
            ...
            tmp = tmp.split("!")
            tmp_label = tmp[1].strip()
            tmp_kpt = [float(i) for i in  tmp[0].split()]
            # import pdb; pdb.set_trace()
            if len(high_kpt) == 0:
                high_kpt.append(tmp_kpt)
                high_label.append(tmp_label)
            else:
                if high_label[-1] == tmp_label:
                    continue
                else:
                    high_kpt.append(tmp_kpt)
                    high_label.append(tmp_label)
    return high_label,np.array(high_kpt)
